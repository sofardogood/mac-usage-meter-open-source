...
<array>
    <string>identifier "com.macusagemeter.MacUsageMeter" and anchor apple generic and certificate leaf[subject.CN] = "Developer ID Application: YOUR_NAME (TEAM_ID)"</string>
</array>
```
...
<dict>
    ...
    <string>identifier "com.macusagemeter.helper" and anchor apple generic and certificate leaf[subject.CN] = "Developer ID Application: YOUR_NAME (TEAM_ID)"</string>
</dict>
```

### 注意事項

- `certificate leaf[subject.CN]` の値は署名に使用する Developer ID 証明書の CN（Common Name）と完全一致させること
- 開発時にワイルドカード `"Developer ID Application: *"` を使っている場合、リリース前に実際の証明書名に置き換えること
